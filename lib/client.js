window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function Summary({ api, t, wide, useSessions, models, sessions }) {
      ...
      const title = [allowance, data?.state === "ready" ? `${line} ${t("remaining")}` : t(data?.state ?? "loading"),
      ...
      return h("button", { type: "button", onClick: refresh, disabled: busy, title, "aria-label": `${title}. ${t("refresh")}`,
        ...
        wide === false ? h("span", null, spark ? "Spark" : "Codex", h("br"), amount(quota?.fiveHour, data), h("br"), amount(quota?.weekly, data)) :
          h(React.Fragment, null, h("div", { style: { fontWeight: 600, overflowWrap: "anywhere" } }, `${allowance} · ${t("remaining")}`),
    ...
    async function apply(ctx) {
      ...
      ctx.inject(["remote.codexUsage", "modelDirectories", "sessions"], scope => {
        const inject = () => ({ api: scope.remote.codexUsage });
        const sidebarInject = () => ({ ...inject(), models: scope.modelDirectories, sessions: scope.sessions });
